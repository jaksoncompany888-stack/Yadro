...
class StepExecutor:
    ...
    def _build_smm_prompt(
        ...
        if purpose == "smm_generate_post":
            system_prompt = """Ты SMM-копирайтер. Пишешь посты для Telegram.
...
ЗАПРЕЩЕНО (звучит как AI):
- "знаменует новую эру", "является свидетельством", "служит примером"
- "не просто X, это Y" — такие конструкции
- "в современном мире", "в наше время" — пустые фразы
- "эксперты отмечают" без конкретных имён
- Списки ровно из 3 пунктов без причины
- Повторять одну мысль разными словами подряд

ПИШИ КАК ЖИВОЙ ЧЕЛОВЕК:
- Прямо обращайся к читателю
- Конкретные цифры и факты вместо "многие", "некоторые"
- Можно задать вопрос в конце
...
ОБЯЗАТЕЛЬНО:
1. Копируй СТИЛЬ из анализа каналов — длина, тон, формат, эмодзи
2. Если в стиле есть эмодзи — ИСПОЛЬЗУЙ эмодзи (🔥 💡 ✨ и т.д.)
3. Учитывай ПРАВКИ КЛИЕНТА — это его предпочтения
4. Вдохновляйся ПРИМЕРАМИ которые зашли
